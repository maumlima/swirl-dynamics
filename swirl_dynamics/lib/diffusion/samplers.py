...
@flax.struct.dataclass
class CustomSampler:
  """Base class for noising-denoising-based diffusion samplers from an original sample.
  ...
  def generate(
    ...
    Args:
    init_sample: The initial sample from which to generate the diffusion samples.
      num_samples: The number of samples to generate in a single batch.
      rng: The base rng for the generation process.
      cond: Explicit conditioning inputs for the denoising function. These
        should be provided **without** batch dimensions (one should be added
    ...
    x0 = jnp.stack([init_sample] * num_samples, axis=0)

    if cond is not None:
      cond = jax.tree.map(lambda x: jnp.stack([x] * num_samples, axis=0), cond)

    noise_rng, denoise_rng = jax.random.split(rng)

    # Forward pass
    x1 = self.noise(x0, noise_rng, self.tspan[::-1])
  ...
  def noise(
    ...
    Args:
      x0: A batch of the initial sample num_samples times.
      rng: Base Jax rng for denoising.
      tspan: An increasing sequence of diffusion time steps within the interval
      (0, t_star].
  ...
  def denoise(
    ...
    Args:
      ...
      rng: Base Jax rng for denoising.
      tspan: A decreasing sequence of diffusion time steps within the interval
        [t_star, 0). The first element aligns with the time step of the `noisy`
        input.
      cond: (Optional) Conditioning inputs for the denoise function. The batch
        dimension should match that of `noisy`.
      guidance_inputs: Inputs for constructing the guided denoising function.
...
@flax.struct.dataclass
class SdeCustomSampler(CustomSampler):
  """Draws samples by solving the forward and then the backward SDE from a given sample.
  ...
  def noise(
    ...
    """Applies iterative noising to given initial states."""
    if self.integrator.terminal_only and self.return_full_paths:
      raise ValueError(
          f"Integrator type `{type(self.integrator)}` does not support"
          " returning full paths."
    ...
    denoised = self.integrator(self.forward_dynamics, x0, tspan, rng, params)
    # SDE solvers may return either the full paths or the terminal state only.
    # If the former, the lead axis should be time.
    samples = denoised if self.integrator.terminal_only else denoised[-1]
    return denoised if self.return_full_paths else samples
  ...
  @property
  def forward_dynamics(self) -> sde.SdeDynamics:
    """Drift and diffusion terms of the forward SDE.

    Following Karras et al. (https://arxiv.org/abs/2206.00364):

      dx = ṡ(t)/s(t) x dt + s(t) √[2σ̇(t)σ(t)] dωₜ,
  ...
  def denoise(
    ...
    denoised = self.integrator(self.backward_dynamics, noisy, tspan, rng, params)
    # SDE solvers may return either the full paths or the terminal state only.
    # If the former, the lead axis should be time.
    samples = denoised if self.integrator.terminal_only else denoised[-1]
    return denoised if self.return_full_paths else samples
  ...
  @property
  def backward_dynamics(self) -> sde.SdeDynamics:
    """Drift and diffusion terms of the backward SDE.
