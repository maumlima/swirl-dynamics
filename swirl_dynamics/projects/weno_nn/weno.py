...
def upwind_weights(order: int = 3) -> tuple[Array, Array]:
    ...
    if order not in [3, 5]:
        raise ValueError(
            "Only 3rd and 5th order polynomials are supported, not ", f"order {order}."
        )
...
def weno_interpolation_plus(
    u_bar: Array,
    omega_fun: Callable[[Array, int | None], Array],
    order: int = 3,
) -> jnp.float64:
    ...
    assert len(u_bar) == order, (
        "Input size and order do not match. They should",
